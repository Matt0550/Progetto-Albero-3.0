...
SoftwareSerial mySerial(D5, D6); // RX, TX on Arduino
...
void post_put_leds() {
    ...
    if (!jsonBody.success()) {
        ...
        if (http_rest_server.method() == HTTP_POST) {
          ...
          Serial.println((String)"Led_"+led_resource.gpio+"_"+led_resource.status);
          mySerial.println((String)led_resource.gpio+"_"+led_resource.status);
...
void setup(void) {
    ...
    for(int i = 0; i < pinGPIOLEN; i++) {
      int state = digitalRead(pinGPIO[i]);
      Serial.print((String)"Led "+pinGPIO[i]+": "+state);
    }
}
